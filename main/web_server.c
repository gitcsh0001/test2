...
".trip{color:#c00;font-weight:bold}.ok{color:#0a0}"
"</style></head><body>"
"<h2>ESP32 Modbus Slave</h2>"
"<div>current: <span id='cur'>...</span></div>"
"<fieldset><legend>Anti-backflow</legend>"
"<div>Active power: <b id='pw'>?</b> W</div>"
"<div>Export limit: <b id='lim'>?</b> W</div>"
"<div>Fail-safe: <span id='fs'>?</span></div></fieldset>"
"<form id='f'>"
"<label>Mode</label>"
...
"<input name='token' id='token' type='password' placeholder='required to apply'>"
...
"pw.textContent=s.power;lim.textContent=s.limit;"
"var t=(s.status&1);fs.textContent=t?'TRIP':'OK';fs.className=t?'trip':'ok';"
...
static esp_err_t status_get(httpd_req_t *req)
{
    ...
    int n = snprintf(buf, sizeof(buf),
        ...
        "\"online\":%d,\"power\":%d,\"limit\":%d,\"status\":%u}",
        ...
        (int)modbus_power_w(), (int)modbus_limit_w(), modbus_status());
...
static esp_err_t config_post(httpd_req_t *req)
{
    ...
    /* Access-token check: protects configuration of grid-connected gear. */
#if CONFIG_MB_WEB_AUTH_ENABLE
    char tok[64] = {0};
    if (httpd_query_key_value(body, "token", tok, sizeof(tok)) != ESP_OK ||
        strcmp(tok, CONFIG_MB_WEB_AUTH_TOKEN) != 0) {
        ESP_LOGW(TAG, "Rejected config change: bad/missing token");
